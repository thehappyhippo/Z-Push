...
class BackendCalDAV extends BackendDiff {
    ...
    public function Logon($username, $domain, $password) {
        ...
        if  (defined('CALDAV_USE_FULLEMAIL_FOR_PATH') && !CALDAV_USE_FULLEMAIL_FOR_PATH) {
             $username_plain = Utils::GetLocalPartFromEmail($username);
        }
        $this->_caldav_path = str_replace('%u', $username_plain, CALDAV_PATH);
        $url = sprintf("%s://%s:%d%s", CALDAV_PROTOCOL, CALDAV_SERVER, CALDAV_PORT, $this->_caldav_path);
        $this->_caldav = new CalDAVClient($url, $username_plain, $username, $password);
